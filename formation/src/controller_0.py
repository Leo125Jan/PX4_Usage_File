...
def	controller():
	...
	# Position Control
	cmd_vel.linear.x = 0.5*((P1[0] - P0[0]) + 2.0 + (P2[0] - P0[0]) + 1.0) + 0.5*(P3[0] - P0[0] + 1) + 1*P3_v[0]
	cmd_vel.linear.y = 0.5*((P1[1] - P0[1]) + 0.0 + (P2[1] - P0[1]) - sqrt(3)) + 0.5*(P3[1] - P0[1] - 0.5*sqrt(3)) + 1*P3_v[1]
	cmd_vel.linear.z = 0.5 - P0[2]
...
if __name__ == '__main__':
	...
	try:
		...
		px4_0 = Px4Controller("iris_0")
		rospy.Subscriber('/gazebo/model_states', ModelStates, odom, queue_size = 10)
		rospy.Subscriber("/iris_0/mavros/imu/data", Imu, imu_callback, queue_size = 10)
